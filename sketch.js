...
function setup() {
  ...
  r = new Square("rot", 1, width/2, height-40);
  b = new Square("blau", 1, width/2-80, height-(height-20));
...
function draw() {  // besser mit einer Klasse + Konstruktor Square mit Attributen wie Farbe und Position, aber erst spaeter
 if (start) {
   ...
   b.left_pos += Math.pow(-1, b.richtung);
...
function Square(color, richtung, left_pos, right_pos) {
 this.color = color;
